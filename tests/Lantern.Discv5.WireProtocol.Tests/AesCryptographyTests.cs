...
public class AesCryptographyTests
{
    ...
    public void AesCtrEncrypt_InvalidKeyOrIvLength_ThrowsArgumentException()
    {
        ...
        Assert.Throws<ArgumentException>(() => AesCryptography.AesCtrEncrypt(invalidMaskingKey, invalidMaskingIv, header));
    }
    ...
    public void AesCtrEncryptDecrypt_ShouldEncryptAndDecryptCorrectly()
    {
        ...
        var header = GenerateRandomBytes(16);
        var encryptedHeader = AesCryptography.AesCtrEncrypt(maskingKey, maskingIv, header);
        Assert.AreNotEqual(header, encryptedHeader);
        
        var decryptedHeader = AesCryptography.AesCtrDecrypt(maskingKey, maskingIv, encryptedHeader);
