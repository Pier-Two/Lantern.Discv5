...
public class UdpCommunication
{
    ...
    public async Task SendAsync(byte[] data, IPEndPoint destination)
    {
        ...
        if (completedTask == timeoutTask) throw new TimeoutException("Send timed out");
    }
    ...
    {
        if (data.Length < MinPacketSize) throw new InvalidDataException("Packet is too small");
        if (data.Length > MaxPacketSize) throw new InvalidDataException("Packet is too large");
    }
    ...
    private async Task<UdpReceiveResult> ReceiveAsyncWithTimeoutAsync()
    {
        ...
        if (completedTask == timeoutTask) throw new TimeoutException("Receive timed out");
